...
router.patch('/tasks/:id', auth, async (req, res) => {
    ...
    const taskAllowUpdates = ['description', 'completed']
    const taskValidation = taskUpdates.every((update) => taskAllowUpdates.includes(update))
    if (!taskValidation) {
        return res.status(400).send({error: 'Invaled Update'})
    }
    try {
